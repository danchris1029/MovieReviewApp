...
public class MovieReviewApp {
    ...
    public static void main(String [] args) {
        ...
        System.out.println("\nDatabase size: " + rh.getDatabase().size() + welcomeMessage);
        ...
        while (!selection.equals("0")) {
            switch (selection) {
                ...
                case "2":
                    ...
                    if (!idStr.matches("-?(0|[1-9]\\d*)")) { // Check that the input is a digit.
    ...
    /**
     * Print the headers of the table content for reviews
     */
    ...
    /**
     * Print table content to output
     * @param mr The MovieReview object that will have its contents printed to the screen
     */
    public static void printTableContent(MovieReview mr) {
    	...
    	information += String.format("%6s", mr.getId()) + " |  ";
    	
    	textScnr = new Scanner(mr.getText());
    	textLine = textScnr.next();
    	
    	while(textScnr.hasNext()) {
    		lastWordInLine = textScnr.next();
    		if(textLine.length()+lastWordInLine.length()+1 > 52 ||textScnr.hasNext() == false)
    			break;
    		textLine += " " + lastWordInLine;
    	}
    	information = information + String.format("%-52s", textLine) + " | ";
    	textLine = "";
    	information += String.format("%-10s | ", mr.getPredictedScore());
    	information += String.format("%-10s |\n", mr.getRealScore());
    	...
    	while(textScnr.hasNext()) {
    		...
    		if(textLine.length()+lastWordInLine.length()+1 > 52 ||textScnr.hasNext() == false)
    		{
    			if(textScnr.hasNext() == false && !(textLine.length()+lastWordInLine.length()+1 > 52))
    			{
    				textLine += " "+lastWordInLine;
    				information += String.format("%9s","| ");
        			information = information + String.format("%-53s", textLine) + " | ";
        			information += String.format("%22s  |\n", "");
        			break;
    			}
    			else if(textScnr.hasNext() == false && textLine.length()+lastWordInLine.length()+1 > 52)
    			{
    				information += String.format("%9s","| ");
        			information = information + String.format("%-53s", textLine) + " | ";
        			information += String.format("%22s  |\n", "");
